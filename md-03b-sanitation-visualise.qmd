...
```{r}
sanitation_hungary |> 
  ggplot(aes(residence, percent, fill = varname_long)) +
  geom_col()+
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? –\> They are ordered by the alphabet.

2.  What would you want to change? –\> Change the legend from least to most improved sanitation.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    --\> Because safely managed is a subgroup of basic sanitation. Therefore, including them would count them twice.

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot? –\> the trend is similiar for all residence types
